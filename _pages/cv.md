...
### Context-aware LLM-based Third-Party Library Upgrade Incompatibility Fixing
Addressing the problem that incompatibility errors caused by breaking changes in library APIs during third-party library upgrades are difficult to fix automatically, we propose Librarian, a context-aware LLM-based automatic fixing approach. It extracts semantic fix hints from the library codebase as few-shot examples to guide LLM in generating fixes, and iteratively refines the fixes based on compilation feedback from the client project.

* **Context-aware LLM-based Third-Party Library Upgrade Incompatibility Fixing Tool (Librarian)** (Core Contributor | ISSTA'26 (CCF-A) Second Author)
  * Addressing the limitations of existing approaches that rely on predefined migration patterns or API recommendation heuristics and ignore the usage context of broken APIs (leading to incorrect or incomplete fixes), Librarian extracts context-aware fix hints from the library codebase as semantic few-shot examples, enabling LLM to generate fixes without relying on predefined patterns; it further performs fix refinement based on compilation feedback from the client project to ensure correctness. Achieved a fixing success rate of 84.2%, outperforming state-of-the-art approaches by at least 45.3%, and demonstrated practical usefulness by fixing incompatibility errors in 32 real-world projects.
...
* **Model Dependency Recovery Tool for Large Model Supply Chains (TensorLock)** (Core Contributor | Ministry of Science and Technology 2030 Major Project | ISSTA'26 (CCF-A) First Author)
